...
object Evernote {
  def props(events:TasksEventBus) = {
...
class Evernote(private val events:TasksEventBus) extends Actor with ActorConfig with ActorLogging{
  ...
  private lazy val user = {
    factory.createUserStoreClient().getUser
  }

  override def receive = {
    case Refresh =>
      log.info("Refreshing tasks from {}", cfg.service)
      find().foreach { task =>
        log.info(s"Found ${task.id} from Evernote")
    ...
    case TaskSaved(task, Some(uri)) =>
      ...
      log.info(s"""Note "${note.getTitle}" tagged with ${note.getTagNames} for $uri""")
  }
  ...
  class Cfg {
    val service = EvernoteService.valueOf(config.getString("providers.evernote.service").toUpperCase())
    val token = config.getString("providers.evernote.token")
  }
}
